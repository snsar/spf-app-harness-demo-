...
| 2026-06-29 | All 5 sub-agents `model: opus`→`sonnet` to cut token cost | .claude/agents/*.md | User request. Note: orchestrator may override `model: opus` per-call on HIGH-risk security/QA gates (F3/F3b/F7/F8) where adversarial depth caught real findings (e.g. F3 empty-secret HIGH). |
